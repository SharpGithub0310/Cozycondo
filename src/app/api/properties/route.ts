...
export async function GET(request: NextRequest) {
  try {
    ...
    (data || []).forEach((prop) => {
      ...
      const sortedPhotoRecords = (prop.property_photos || [])
      ...
      result[slug] = {
        ...
        baseOccupancy: prop.base_occupancy || 2,
        extraPersonFee: parseFloat(prop.extra_person_fee || '0'),
  ...
  try {
    ...
    return await requireAuth(request, async (req, adminSession) => {
      ...
      if (photos.length > 0) {
        ...
        // Insert new photos
        const photoInserts = persistedPhotos.map((photoUrl: string, index: number) => ({
